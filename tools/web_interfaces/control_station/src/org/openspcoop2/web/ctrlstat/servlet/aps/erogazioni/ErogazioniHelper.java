...
public class ErogazioniHelper extends AccordiServizioParteSpecificaHelper{
	...
	//			TipiConnettore.CUSTOM -> connettore custom
		...
		if(gestioneFruitori) {
			...
			if(listaMappingFruzionePortaDelegata.size()>1) {
				List<String> azioniL = new ArrayList<>();
				Map<String,String> azioni = this.porteDelegateCore.getAzioniConLabel(asps, as, false, true, new ArrayList<String>());
				if(azioni != null && azioni.size() > 0)
					azioniL.addAll(azioni.keySet());
				allActionRedefined = this.allActionsRedefinedMappingFruizione(azioniL, listaMappingFruzionePortaDelegata);
			}
...
//					e.addElement(de);
	...
	public void prepareErogazioneChange(TipoOperazione tipoOp, AccordoServizioParteSpecifica asps) throws Exception {
		...
		IDServizio idServizio = IDServizioFactory.getInstance().getIDServizioFromAccordo(asps);
		String tmpTitle = gestioneFruitori ? this.getLabelIdServizio(idServizio) :  this.getLabelIdServizioSenzaErogatore(idServizio);
